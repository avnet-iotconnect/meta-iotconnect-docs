...
[Service]
ExecStart=/bin/bash /home/root/iotc-application.sh
...
ProtectSystem=off
ProtectHome=no
CapabilityBoundingSet=CAP_SYS_ADMIN CAP_DAC_OVERRIDE CAP_SYS_RAWIO CAP_SYS_TTY_CONFIG CAP_NET_BIND_SERVICE
...
update_directory() {
    ...
    # Check if the directory exists on the target
    if ssh $TARGET_USER@$TARGET_IP "[ -d $target_dir ]"; then
        echo "Updating $target_dir on the target device..."
        ssh $TARGET_USER@$TARGET_IP "ls -R $target_dir"
    else
        echo "Warning: $target_dir not found on the target device, skipping update."
    fi
}

validate_target_state() {
    echo "Validating target directories on $TARGET_IP..."
    ssh $TARGET_USER@$TARGET_IP "ls -R /usr/iotc/bin/iotc-python-sdk/model"
...
Note: To update Wi-Fi settings, use 'change_wifi.sh' located in '/usr/iotc/local'.
================================================================================
