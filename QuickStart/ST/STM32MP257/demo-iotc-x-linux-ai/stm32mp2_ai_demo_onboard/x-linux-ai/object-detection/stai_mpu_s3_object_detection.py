...
class Application:
    ...
    def classify_s3_image(self, bucket_name, object_name):
        """Download an image from S3, classify it using NPU, and store the confidence level in S3."""
        ...
        # *** IMAGE PREPROCESSING: Resize image to match NN model input dimensions ***
        img_resized = cv2.resize(img, (self.nn_input_width, self.nn_input_height))
        print(f"Image resized to: {self.nn_input_width}x{self.nn_input_height}")
        ...
        try:
            ...
            # Get results
            results = self.nn.get_results()

            if isinstance(results, list) and len(results) == 2:
                accuracy, label_index = results
            else:
                print("Inference returned unexpected or empty results.")
                return

            # Get the labels from the model
            labels = self.nn.get_labels()
    
            if len(labels) == 0 or label_index >= len(labels):
        ...
        try:
            ...
            print(f"Error writing classification or confidence to files: {e}")
            return

        # Fetch the original filename from S3
        try:
            metadata_key = "metadata/bottom_original_filename.txt"
            response = self.s3.get_object(Bucket=bucket_name, Key=metadata_key)
            original_filename = response['Body'].read().decode('utf-8').strip()
            print(f"Original filename retrieved: {original_filename}")
        except Exception as e:
            print(f"Failed to fetch original filename: {e}")
            return

        # Store the confidence in S3 with the original filename
        confidence_file_name = original_filename.replace('.jpg', '_confidence.txt')
        confidence_key = confidence_folder + confidence_file_name
        confidence_body = f"Confidence: {confidence:.2f}%\n"
        try:
            self.s3.put_object(Bucket=bucket_name, Key=confidence_key, Body=confidence_body)
            print(f"Stored confidence for {original_filename} in {confidence_key}.")
        except Exception as e:
            print(f"Failed to store confidence in S3: {e}")
