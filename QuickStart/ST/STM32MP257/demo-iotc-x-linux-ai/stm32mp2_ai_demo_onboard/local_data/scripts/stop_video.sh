#!/bin/bash
echo "Starting stop_video.sh script" >> /var/log/iotconnect.log
...
/bin/ps aux | /bin/grep 'python3 /usr/local/x-linux-ai/image-classification/stai_mpu_image_classification.py' | /bin/grep -v 'grep' | /usr/bin/awk '{print $2}' | /usr/bin/xargs sudo /bin/kill -9 >> /var/log/iotconnect.log 2>&1
